...
- 캘린더·메모·프로젝트·완료 기록을 한 화면 흐름으로 통합
...
**대시보드 · 노트 · 프로젝트 · 완료 기록 · 설정** 5개 화면으로 구성. 상단 바에서 `질문`, `노트 추가`, `AI 일정 추가`(단축키 `A` 또는 `Ctrl+Shift+N`) 상시 호출 가능.
...
완료 일정의 월별 조회. 최근 1년 활동량, 당월 완료 수, 중요 완료 건수 요약. 제목·내용·프로젝트·종류 기준 검색 지원.

---

## AI 기능

네이티브 function calling 비의존 방식. 프롬프트로 JSON 스키마와 도구 목록을 지시하고, 모델이 반환한 도구 호출을 앱이 로컬 실행한 뒤 결과를 첨부해 재요청(도구 사용 에이전트 최대 3라운드). OpenAI 호환 엔드포인트 전반과 연결 가능하며 로컬 모델도 포함.
