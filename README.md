...
**대시보드 · 나의 루틴 · 노트 · 프로젝트 · 나의 기록 · 설정** 6개 화면으로 구성. 상단 바에서 `질문`, `노트 추가`, `AI 일정 추가`(단축키 `A` 또는 `Ctrl+Shift+N`) 상시 호출 가능.
...
반복되는 일을 규칙으로 보관하고, 챙길 시점에 이번 회차의 일정 생성을 제안합니다. 예를 들어 `영수증 취합 · 매월 25일 · 7일 전 안내`를 등록하면 매월 18일부터 대시보드와 나의 루틴 탭에서 확인할 수 있습니다.

- 매월·N개월마다(1~24개월)·매년 반복, 시작 월·예정일·0~30일 전 안내 설정
- 일정 만들기 제안 또는 알림만 받기. 일정을 실제 저장할 때만 회차를 처리하며 초안 취소는 처리하지 않음
- 내일·3일 뒤·7일 뒤·직접 날짜로 미루기, 이번 회차 건너뛰기, 확인 완료, 일시 중지·다시 시작
- 같은 회차 중복 생성 방지, 처리 이력 및 생성한 일정 바로가기
- 해당 날짜가 없는 달은 말일 적용. 장기간 미접속 시 안내일이 지난 가장 최근 회차 한 개만 제안
- 확장 프로그램에서 설정의 창 표시를 켜면 안내일 오전 9시에 기존 탭으로 알림. 웹/단일 HTML은 앱을 열었을 때와 사용 중 대시보드·탭 배지에서 확인
- 루틴 삭제 시 이미 생성한 일정은 유지. 규칙 수정은 기존 일정과 처리 이력을 변경하지 않음
- 루틴과 처리 이력은 백업 스키마 v6에 포함. v4/v5 가져오기도 지원하지만 과거 백업으로 전체 복원하면 루틴은 빈 목록으로 교체됨. v6 백업은 이 기능을 포함한 최신 앱에서 복원
...
- **v1.3.0 백업** — 5MB 초과 시 ZIP으로 자동 압축하며 JSON·ZIP 모두 가져오기 지원(압축 전 최대 50MB). 내부 자동 백업은 별도 IndexedDB에 최대 20개·합계 100MB까지 보관하고, 기존 브라우저 백업은 저장 검증 후 이전합니다. 새 백업은 구버전에서 지원하지 않을 수 있으므로 복원 시 v1.3.0 이상을 사용하세요.
- 프로젝트에 일정·노트·세부 항목·AI 규칙 참조가 남아 있으면 삭제를 차단합니다. 일정 삭제·실행 취소는 노트 연결도 함께 정리·복원합니다.
- 기간 일정은 진행 중인 날의 브리핑에 포함하고 종료일 기준으로 지연을 판단합니다. 맞닿은 시간 구간은 충돌로 표시하지 않습니다.
...
npm test             # 백업·일정 판정·LLM·마크다운 회귀 검사
npm run test:integration # 격리 Chromium의 IndexedDB·실행 취소 통합 검사
npm run test:routines # 빌드 후 루틴 UI·확장 알림 검사
```
...
| 저장소 | Dexie(IndexedDB) v6 스키마, `useLiveQuery` 반응형 조회 |
